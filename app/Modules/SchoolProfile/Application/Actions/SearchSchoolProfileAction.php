...
class SearchSchoolProfileAction
{
	...
	public function execute(int $Id_School, SearchSchoolProfileDTO $oData) : Result
	{
		//------------------------------------------------------------------------------
		//	VARIABLES
		//------------------------------------------------------------------------------
		...
		try
		{
			...
			$oResult = $this->oSchoolProfileRepository->search($Id_School, $oData);
			if ( $oResult->RESULT_STS <> 200 ){ DB::rollBack(); return $oResult; }
