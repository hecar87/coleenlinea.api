...
class PayEnrollmentInstallmentAction
{
	...
	public function execute(int $Id_EnrollmentInstallment) : Result
	{
		...
		try
		{
			...
			$oResult = $this->oEnrollmentInstallmentRepository->canPay($Id_EnrollmentInstallment);
			if ( $oResult->RESULT_STS <> 200 ){ DB::rollBack(); return $oResult; }

			$oResult = $this->oEnrollmentInstallmentRepository->pay($Id_EnrollmentInstallment);
			if ( $oResult->RESULT_STS <> 200 ){ DB::rollBack(); return $oResult; }
