...
class NullifyEnrollmentInstallmentAction
{
	...
	public function execute(int $Id_EnrollmentInstallment) : Result
	{
		...
		try
		{
			...
			$oResult = $this->oEnrollmentInstallmentRepository->canNullify($Id_EnrollmentInstallment);
			if ( $oResult->RESULT_STS <> 200 ){ DB::rollBack(); return $oResult; }

			$oResult = $this->oEnrollmentInstallmentRepository->nullify($Id_EnrollmentInstallment);
			if ( $oResult->RESULT_STS <> 200 ){ DB::rollBack(); return $oResult; }
